...
Many things can be added, more metrics, more UP DOWN indicators quantifying the change of last day/week/month, plotting the data of 6 months ago or a year ago for direct comparison of historical values. More relative change plots, like the [[http://vis.stanford.edu/protovis/ex/index-chart.html][protovis index-chart]] are very useful, even when they are not dynamic, like the one i have on the dashboard above.

Also useful would be confidence intervals asserting how sure are the changes, checking that they are not just by random chance. And of course, next step is to add fits to data, either regression line, monte carlo, etc... that smooth out all the variation in graphs to better understand the overal tendency and also allow to predict the next day/week/month values.
...
Uses highcharts as the javascript charting engine, which i can only say good things about, very nice looking and with some useful user interaction functionalities.
