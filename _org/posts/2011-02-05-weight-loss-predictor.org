...
But of course, like with all diets, I'm quickly growing impatient of when i'm going to reach my goal ([[http://www.wolframalpha.com/input/?i=body+mass+index&a=*C.body+mass+index-_*Formula.dflt-&a=*FS-_**BodyMassIndex.BMI-.*BodyMassIndex.H-.*BodyMassIndex.W--&f3=74+kg&f=BodyMassIndex.W_74+kg&f4=176+cm&f=BodyMassIndex.H_176+cm&a=*FVarOpt.1-_**-.***BodyMassIndex.S---.*--][of adequate BMI]]), so lets use R and monte carlo simulations to generate predictions and understand better what to expect.
...
# simulate it 10k times
mcDays74 = replicate(10000, days.to.weight(74, lastweight, kgdelta))
...
#+begin_src R :session R :results graphics :file /my/al3xandr3.github.com/img/w-loss-74.png
# add dates to it, from today's date + #days
days74 = sort(Sys.Date() + mcDays74)

# get the ecdf values into a dataframe
days74.ecdf = summarize(data.frame(days=days74), days = unique(days), 
                        ecdf = ecdf(days)(unique(days)))

# date where its 85% sure i'll reach goal
prob85 = head(days74.ecdf[days74.ecdf$ecdf>0.85,],1)

# plot
ggplot(days74.ecdf, aes(days, ecdf)) + geom_step() +
       ylab("probability") + 
       geom_point(aes(x = prob85$days, y = prob85$ecdf)) +
       geom_text(aes(x = prob85$days, y = prob85$ecdf, 
                    label = paste("85% sure to be 74kg on",
                            format(prob85$days, "%a, %d %b %Y"))), 
                     hjust=-0.04)
#+end_src

#+results:
[[file:/my/al3xandr3.github.com/img/w-loss-74.png]]

[[http://al3xandr3.github.com/img/w-loss-74.png]]
